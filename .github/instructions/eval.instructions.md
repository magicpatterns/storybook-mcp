...
│       │   ├── components.json      # Component manifest for @magicpatterns/storybook-mcp
│       │   └── docs.json            # Optional docs manifest for @magicpatterns/storybook-mcp
...
2. **Storybook MCP - Dev** (`--context storybook-dev`):
   - Sets up Storybook in the trial project with `@magicpatterns/storybook-mcp-addon`
...
3. **Storybook MCP - Docs** (`--context components.json`):
   - Provides component documentation via `@magicpatterns/storybook-mcp` package
...
3. **Optional: Add context files:**
   - `manifests/components.json` - Component manifest for `@magicpatterns/storybook-mcp` (in a `manifests/` subdirectory)
   - `manifests/docs.json` - Optional docs manifest for `@magicpatterns/storybook-mcp` (in a `manifests/` subdirectory)
...
1. Copies the grading template with `.storybook` config (includes `@magicpatterns/storybook-mcp-addon`)
...
4. Provides MCP config to the agent:
   ...
   ```
5. Agent receives MCP tools from `@magicpatterns/storybook-mcp-addon`
...
2. Creates `.mcp.json` in project with stdio server config:
   ...
   ```
3. Agent receives MCP tools from `@magicpatterns/storybook-mcp` package
