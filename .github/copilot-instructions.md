...
- **`packages/addon-mcp`**: Storybook addon using `tmcp`, exposes MCP server at `/mcp` via Vite middleware
  ...
  - **Imports and reuses tools from `@magicpatterns/mcp` package** for component manifest features
...
The `@magicpatterns/mcp` package (in `packages/mcp`) is framework-agnostic:
...
**Option 2: Reuse tools from `@magicpatterns/mcp`** (for component manifest features):

1. Import the tool from `@magicpatterns/mcp` in `src/mcp-handler.ts`:

```typescript
import { addMyTool, MY_TOOL_NAME } from '@magicpatterns/mcp';
```
...
- `addon-mcp` depends on `@magicpatterns/mcp` (workspace dependency)
...
- Component manifest tools are conditionally registered based on feature flags:
  ...
  - Only registers `addListAllDocumentationTool`, `addGetDocumentationTool`, `addGetStoryDocumentationTool`, `addReadComponentCodeTool`, `addGetDesignTokensTool`, and `addGetDesignGuidelinesTool` when enabled
...
When making changes to `@magicpatterns/mcp` or `@magicpatterns/addon-mcp` that affect users (bug fixes, features, breaking changes, dependency updates), you **MUST** create a changeset:
...
3. File format:
   ...
   ```markdown
   ---
   '@magicpatterns/mcp': patch
   '@magicpatterns/addon-mcp': patch
   ---
