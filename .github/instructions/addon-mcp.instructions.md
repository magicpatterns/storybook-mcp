...
# Copilot Instructions for @magicpatterns/storybook-mcp-addon
...
2. **Factory Pattern**: `createAddonMcpHandler()` creates configured handler instances following the pattern from `@magicpatterns/storybook-mcp`
...
**For reused tools from `@magicpatterns/storybook-mcp`**: The addon uses optional handlers (`onListAllDocumentation`, `onGetDocumentation`) provided by the `StorybookContext` to track usage. These handlers are passed in the context when calling `transport.respond()` in `mcp-handler.ts`:
...
Published to npm as `@magicpatterns/storybook-mcp-addon`.
...
**Note**: This addon shares the same MCP architecture as `@magicpatterns/storybook-mcp` package but is specifically designed to run within a Storybook dev server environment. The main difference is the integration layer - this addon uses Vite middleware while the standalone package provides a pure HTTP handler.
